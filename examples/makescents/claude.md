...
- verification — OTP flow, Twilio Verify, phone validation
- data submission — form submit endpoint, Google Sheets write, broker SMS
- credentials — any file referencing env vars, API keys, or secrets
- championware embed — standalone page URL, embed method, or iframe config NEW
...
## Stack
- Frontend: React (embedded in Championware + standalone page)
- Backend: Serverless functions (Vercel)
- Verification: Twilio Verify — SMS OTP, alphanumeric sender ID (ACMA approved)
- Broker alerts: Twilio Programmable SMS (separate from Verify)
- Data: Google Sheets via Service Account (not OAuth)
- QR code: SVG (print) + high-res PNG (digital), points to standalone URL
...
- spec.md — full project spec including form fields, data flow, and out of scope.
